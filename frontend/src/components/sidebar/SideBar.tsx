...
const UserMenu: FC<UserMenuProps> = ({ open, onClose }) => {
  ...
  return (
      ...
      >
        <ul className="py-2 text-sm">
          {USER_MENU_ITEMS.map(({ title, icon: Icon, url }) => {
            if (title === "Cerrar sesión") {
              ...
              return (
                  <li
                      ...
                      className="px-4 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer flex items-center gap-2"
                  >
                    <Link to={url} className="flex items-center gap-2 w-full" onClick={onClose}>
                      <Icon className="w-4 h-4" />
                      <span>{title}</span>
                    </Link>
                  </li>
